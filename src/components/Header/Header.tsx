...
const Header = () => {
	...
	const MaterialUISwitch = styled(Switch)(({ theme }) => ({
		...
		'& .MuiSwitch-switchBase': {
			...
			'&.Mui-checked': {
				...
				'& + .MuiSwitch-track': {
					opacity: 1,
					backgroundColor: theme.palette.mode === `${themeDark ? 'dark' : 'light'}` ? '#8796A5' : '#aab4be',
				},
			},
		},
		'& .MuiSwitch-thumb': {
			backgroundColor: theme.palette.mode === `${themeDark ? 'dark' : 'light'}` ? '#003892' : '#001e3c',
			width: 32,
			height: 32,
			'&:before': {
				content: "''",
	...
	return (
		<header id='Header' className={`${css.header} container ${isScrolled ? css.scrolled : ''}`}>
				...
				<div className={css.header_left}>
					{modalMobile ? (
						<>
							...
							<Button onClick={() => i18n.changeLanguage(i18n.resolvedLanguage === 'en' ? 'ru' : 'en')} text={<img className={css.darkLightMode_icon} src={themeDark ? LanguageDark : LanguageLight} alt='language' />} />
						</>
					) : null}

					<Button onClick={() => toggle(setModalMobile)} text={<img src={themeDark ? burger_icon_dark : burger_icon_light} alt='search' />} />
					<Button text={<img src={themeDark ? SearchDark : SearchLight} alt='search' />} />
				</div>
				<div className={`${css.modal_header} ${modalMobile ? css.open_modal : null}`}>
